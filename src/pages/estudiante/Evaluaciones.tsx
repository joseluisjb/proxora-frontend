...
export default function Evaluaciones() {
  ...
  const cargar = useCallback(async () => {
    ...
    setError(null);
    try {
      const result = await evaluacionesService.listarMisProyectos(usuario.id, { size: 100 });
    ...
    <div className="animate-fade-in">
      <div className="mb-6 animate-slide-up">
        ...
        <p className="text-[13px] text-[#6B7280] max-w-[480px]">
          Revisa todas las evaluaciones y retroalimentación recibidas para tus proyectos académicos.
        </p>
      </div>
      ...
      {!cargando && !error && proyectos.length === 0 && (
        <div className="flex flex-col items-center justify-center py-24 gap-3">
          ...
          <p className="text-[14px] text-[#6B7280]">No tienes proyectos con evaluaciones aún.</p>
        </div>
      )}
            ...
            {proyectosPaginados.map((p, i) => {
              ...
              return (
                ...
                >
                  ...
                  <button
                    type="button"
                    onClick={() => navigate(`/estudiante/evaluaciones/${p.id}`)}
